...
if __name__ == "__main__":
    ...
    for filename in sorted(glob.glob("./largestclustersizevstime_data/volfracribo"+str(volume_fraction_ribosome)+"_Tc"+str(crowder_temperature)+"_eps*_medianlargestclustersize.data"),key=lambda x:(float((os.path.basename(x).split("_")[2]).replace('eps','')))):
    ...
    plt.grid(alpha=0.4)
    fig.tight_layout()
    plt.savefig('final_figures/largestclustersize_vs_time/volfracribo'+str(volume_fraction_ribosome)+'_Tc'+str(crowder_temperature)+'_largestclustersizevsepsilon.svg',bbox_inches='tight')
    plt.close()
